...
class AuthorProvider extends MongoProvider
  ...
  doInsert: (callback) ->
    ...
    this.insert [{'name': 'William Shakespeare', 'email': 'william@shakespeare.com', 'age': 587},
      {'name': 'Jorge Luis Borges', 'email': 'jorge@borges.com', 'age': 123}], (err, docs) ->
        docs.forEach (doc) ->
      ...
      authorProvider.find {}, {'sort': [['age', 1]]}, (err, cursor) ->
        cursor.each (err, author) ->
          if (author != null) 
            ...
            userProvider.insert [{'login': 'jdoe', 'name': 'John Doe', 'email': 'john@doe.com'},
              {'login': 'lsmith', 'name': 'Lucy Smith', 'email': 'lucy@smith.com'}], (err, docs) ->
                docs.forEach (doc) ->
                  sys.puts(sys.inspect(doc))
                  users[doc.login] = doc
